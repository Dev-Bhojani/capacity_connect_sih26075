...
const MyLearning = () => {
  ...
  const fetchMyLearning = async () => {
    ...
    try {
      ...
      const enrollments = getLearnerEnrollments(CURRENT_LEARNER_ID);
      ...
      /* -------------------------------------------------------
         STEP 3
         Fetch all corresponding courses
      ------------------------------------------------------- */
        ...
        enrollments.map(async (enrollment) => {
          try {
            const response = await fetch(
              `${COURSES_API_URL}/${enrollment.courseId}`,
            );

            if (!response.ok) {
              throw new Error(
                `Unable to load course ${enrollment.courseId}.`,
              );
            }
      ...
      /* -------------------------------------------------------
         STEP 4
         Keep combined enrollment + course data
      ------------------------------------------------------- */

      setLearningData(courseResults);
    } catch (fetchError) {
